...
export default function ProductPage({ initialProduct, initialRelated }) {
  ...
  return (
    <main className="catalog-page product-detail-page">
      <section className="product-hero">
        <div className="container product-hero__grid">
          ...
          <div className="product-hero__copy">
            ...
            <div className="product-selectors" style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
              <label style={{ display: 'flex', flexDirection: 'column', gap: '5px', flex: 1, minWidth: '120px' }}>
                <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>Quantity</span>
                <input type="number" value={quantity} onChange={e => setQuantity(e.target.value)} min="1" style={{ padding: '8px 12px', border: '1px solid var(--border-color)', borderRadius: '4px' }} />
              </label>
              {(product?.attributes?.color?.length > 0 || product?.attributes?.size?.length > 0) && (
                <label style={{ display: 'flex', flexDirection: 'column', gap: '5px', flex: 1, minWidth: '120px' }}>
                  <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>Style / Size</span>
                  <select value={selectedStyle} onChange={e => setSelectedStyle(e.target.value)} style={{ padding: '8px 12px', border: '1px solid var(--border-color)', borderRadius: '4px' }}>
                    <option value="">Select option</option>
                    {product.attributes?.color?.map(c => <option key={c} value={c}>{c}</option>)}
                    {product.attributes?.size?.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </label>
              )}
            </div>

            <div className="catalog-actions" style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
              <button
              ...
              <button
                onClick={() => router.push(`/customize/${params.subcategory}?productId=${product._id || product.id}`)}
                className="catalog-btn catalog-btn--dark"
              >
                Customize Now
              </button>
            </div>
      ...
      <EnquiryModal 
        ...
        image={resolveImage(getImagePath(product))}
